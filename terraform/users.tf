...
resource "azurerm_role_assignment" "store_manager_storage_blob_data_reader" {
  scope                = azurerm_storage_account.prod_data.id
  role_definition_name = "Storage Blob Data Reader"
  principal_id         = azuread_service_principal.storage_manager_sp.id
}

resource "azurerm_role_assignment" "store_manager_storage_account_reader" {
  scope                = azurerm_storage_account.prod_data.id
  role_definition_name = "Reader"
  principal_id         = azuread_service_principal.storage_manager_sp.id
}

resource "azurerm_role_assignment" "current_user_storage_blob_data_reader" {
  scope                = azurerm_storage_account.prod_data.id
  role_definition_name = "Storage Blob Data Reader"
